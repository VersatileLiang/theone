...
public class NodeGraphic extends PlayFieldGraphic {
	...
	private static Color rangeColor1 = Color.GREEN;
	private static Color rangeColor2 = Color.RED;
	private static Color rangeColor3 = Color.yellow;
		...
		if (drawCoverage && node.isRadioActive()) {
			...
			for (NetworkInterface ni : interfaces) {
				...
				coverage = new Ellipse2D.Double(scale(loc.getX()-range),
						...
						scale(range * 2)); 
				if(node.name.startsWith("RSU")){
					g2.setColor(rangeColor2);
				}
				else if(node.name.startsWith("p")){
					g2.setColor(rangeColor3);
				}
				else{
					g2.setColor(rangeColor1);
				}
