...
{   
	...
	public Coord MYStartPoint=null;
	...
	public static final String TORRouter_NS = "MyRouter";
...
//		if(this.getHost().movement.toString().contains("ShortestPathMapBasedMovement"))
        if(this.getHost().movement.toString().contains("MyMovement"))
        {
			/****0 = Shortest Path****/
			...
			else if (this.BuildTrajectoryType==2)
			{
			    ...
			    getAdvancedMobilityMY(msg, Input);
			}
	...
	public void getShortestMY(Message M)
	{
	...
	public void getAdvancedMobilityMY(Message M, int Input)
	{
		M.msgTRA.add(this.getHost().getLocation().clone());
	...
	public void getMobilityMY(Message M)
	{
	...
	public  Tuple<Message, Connection> tryOtherMessages() 
	{
		...
		for(Connection C : getConnections()) 
		{
	        ...
	        MyRouter OtherRouter = (MyRouter)Other.getRouter();
		    	...
		    	if(this.IsNodeMobile()&&OtherRouter.IsNodeMobile())
		    	{
		    		...
		    		else if ((TrajectoryAssociation)&&(OtherRouter.TrajectoryAssociation)&&(OtherRouter.IsWithTrajectory(M).equals(this.IsWithTrajectory(M))))	
		    		{
		    			...
		    			if((this.getVerticalDistance(Input)!=0)&&(OtherRouter.getVerticalDistance(Input)!=0)) {
		    				if((OtherRouter.checkTriangleAngle(Input)==true)&&(OtherRouter.getTrajectoryDiversity(Input)<OtherRouter.getMaxTrajectoryDiversity(Input))) {
		    					MidPriority.add(new Tuple<Message, Connection>(M, C));   
		    				}
		    			}
	...
	public int getTrajectoryForwardingProgress(List<Coord> List, Message message)
	{
		Coord TargetCoord=List.get(1);
		
		for(int i=0; i<=(message.msgTRA.size()-1); i++)
		{
			if(message.msgTRA.get(i).equals(TargetCoord))
				return i;
		}
	...
	{	
		...
		MyRouter otherRouter =(MyRouter)host.getRouter();
	...
	{		
		...
		{	
			...
			MyRouter OtherRouter= (MyRouter) otherHost.getRouter();
	...
	private class MidTupleComparator implements Comparator <Tuple<Message, Connection>> 
	{
		public int compare(Tuple<Message, Connection> Tuple1, Tuple<Message, Connection> Tuple2) 
		{
			...
			MyRouter R1 = (MyRouter)Tuple1.getValue().getOtherNode(getHost()).getRouter();
			MyRouter R2 = (MyRouter)Tuple2.getValue().getOtherNode(getHost()).getRouter();
